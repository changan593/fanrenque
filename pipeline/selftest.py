...
def test_narration_ledger() -> None:
    ...
    # 短旁白整个嵌在一条**心理活动**里时，`others` 挡不住（那条不是旁白），
    # 只能靠段号：承载本条的镜头一定写着本条的段号。
    # E34 seq137[90]「一个大巴掌。」嵌在 seq137[93]「其实不是一个大巴掌，
    # 更像是扔开什么垃圾一样。」里，后者是 monologues，于是 [90] 被算了两次。
    slap = "一个大巴掌。"
    m_slap = [("140", "卡", "一个大巴掌。[90]"),
              ("143", "现", "其实不是一个大巴掌，更像是扔开什么垃圾一样。[93]")]
    check("正向包含也要查段号", len(s14.match(slap, m_slap, 90)) == 1)
    check("不查段号时正向包含会多算一处", len(s14.match(slap, m_slap)) == 2)

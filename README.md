# fanrenque

把网络小说《穿越后，系统变成白噪音了怎么办》（码字的画家 著，1200 个分析单元，正文 268 万字）
改编成 **AI 漫剧**：**6 季 267 集**，每集 2~5 章。**时长不做预设**——一集多长是内容量的结果，
不是先定后写的指标（见 [`doc/00_项目总纲.md`](doc/00_项目总纲.md) 第二节）。

## 三大原则

1. **最终目标是高质量成片** —— 所有环节谨慎细致，人物场景高一致性
2. **严格遵守原文** —— 原文有多少人物场景就多少，怎么描写就怎么呈现，
   对话、心理活动、关键旁白一个不漏
3. **画面风格朴素真实** —— 严禁浮夸

三条原则在整个项目里是硬约束，下游任何环节都不许绕过。原则二落实成了三道程序闸门，不靠模型自觉：
章节层 `s3`（每条引用回原文逐字核验 + 台词覆盖率）、剧本层 `s14`（每条关键旁白都要在剧本里有承载）、
取证层 `s17`（角色卡 / 场景卡 / 幕文档里每处 `【原】「…」` 都逐字落在所标段号）。
一条命令跑全部闸门：`python pipeline/check_all.py`。

---

## 我想…… 该看哪份文档

| 我想…… | 看这里 |
| --- | --- |
| **接手这个项目**（换人 / 换会话 / 隔了很久回来） | [`doc/10_交接说明.md`](doc/10_交接说明.md) ← **从这份开始** |
| 搞懂某个目录 / 文件 / JSON 字段是什么意思 | [`doc/09_数据字典.md`](doc/09_数据字典.md) |
| 看到一个词或一个编号（`seq` `C09` `S01E07` `【现】`）不知道指什么 | [`doc/14_术语与编号表.md`](doc/14_术语与编号表.md) |
| 知道现在做到哪、下一步做什么、什么被卡住 | [`doc/03_路线图.md`](doc/03_路线图.md) |
| 了解项目的目标、规格、六个阶段 | [`doc/00_项目总纲.md`](doc/00_项目总纲.md) |
| 动原文数据前先避坑 | [`doc/01_原文数据说明.md`](doc/01_原文数据说明.md) |
| 知道逐章分析是怎么做的、质量闸门怎么定的 | [`doc/02_章节分析规范.md`](doc/02_章节分析规范.md) |
| **写或改一集剧本** | [`doc/13_剧本格式规范.md`](doc/13_剧本格式规范.md)（格式）+ [`doc/05_唐假旁白系统.md`](doc/05_唐假旁白系统.md)（旁白怎么派） |
| 知道第一季写作里查出了哪些抽取误判、数据哪里不可信 | [`doc/15_写作台账.md`](doc/15_写作台账.md) |
| 出图 / 写分镜时对齐画面风格 | [`doc/04_风格规范.md`](doc/04_风格规范.md) |
| **开始出图、验收卡图、记台账** | [`doc/16_出图工序规范.md`](doc/16_出图工序规范.md) |
| **直接拿去用的风格提示词**（环境 / 人物） | [`production/style_assets/README.md`](production/style_assets/README.md) |
| 某个角色长什么样、跨集怎么锁住不漂 | [`production/characters/README.md`](production/characters/README.md) |
| 看全书剧情线、六季怎么划的、分集表 | [`doc/08_全书剧情线.md`](doc/08_全书剧情线.md) |
| **开始出图**（工序、命名、台账、验收、返工） | [`doc/13_出图工序规范.md`](doc/13_出图工序规范.md) ← **开拍前必读** |
| 跑脚本 / 排查跑批故障 | [`pipeline/README.md`](pipeline/README.md) |
| 复盘当初为什么选国风三维 | [`doc/archive/`](doc/archive/) |
| 知道最近改了什么、为什么改 | [`CHANGELOG.md`](CHANGELOG.md) |

---

## 当前进度

| 阶段 | 状态 |
| --- | --- |
| ① 原文标准化 | ✅ 1200 个分析单元。⚠ 第 1115~1205 章之间 41 章正文被截，待补，见 [`doc/11`](doc/11_待补原文清单.md) |
| ② 逐章独立分析 | ✅ 1200/1200，逐字命中率 **100%**、台词覆盖率 **100%**、臆造 **0**，`s3` 零问题。其中 4 章（ch0990 / ch1149 / ch1155 / ch1167）结构分 82 人工放行，理由在各章 `quality.waiver` |
| ③ 资产与结构规划 | ✅ 997 角色 / 1364 场景 / 3000 对同框 / 90 个卷段 / 6 季 267 集 |
| ④ 风格定稿与资产深化 | ⏳ **画风已定稿（国风三维）**。角色卡 15 具名 + 5 群像、场景卡 10，E01 全覆盖；第一季共 207 角色（主要 25）/ 274 场景待做 |
| ⑤ 第一季深化 | ⏳ 47 集剧本**声音层**全部完成（485 幕 / 12,958 镜，`s14` 全绿）；**画面提示词层** 77 幕（E01~E10）进行中；分镜草图只有 E01 |
| ⑥ 第一季制作 | ⏳ E01 纸面就绪（9 幕 212 镜提示词、角色卡 16 具名 + 5 群像、场景卡 10 母版 + 13 状态），**卡图人工验收进行中，通过 0**。出图工序与台账见 [`doc/16`](doc/16_出图工序规范.md)，闸门 `s18` |

画风载体是**国风三维**——中国院线三维 CG 动画电影，风格化写实。两份统一提示词（环境一份、人物一份）见
[`production/style_assets/`](production/style_assets/README.md)，落槌理由见 [`doc/04`](doc/04_风格规范.md) 第 3.1 节。

**当前的活**：E01 的角色卡与场景卡，提示词和 PNG 都已就位，卡在**人工验收**这一步——验收通过前，图不具备权威
（[`production/characters/README.md`](production/characters/README.md)「PNG 现状」）。已查出三张不过并重写了提示词：
`C04_红儿` V6、`C02_南红枝` V4（脸偏圆、缺特色）、`C03_老拐子` V2（右脚没画出萎缩内翻）；`C53_唐假` 新建卡，Q 版 PNG 待出。
再往后仍是量：第一季 207 角色 / 274 场景，工作队列见 [`data/plot/season_roster.md`](data/plot/season_roster.md)；
开工一幕用 `python pipeline/s16_shot_pack.py`。

---

## 目录

每个目录一句话；每个文件、每个字段的详细含义见 [`doc/09_数据字典.md`](doc/09_数据字典.md)。

```
source/          原文。★只读，任何脚本都不许改它★
  ├── *.txt              抓下来的原始文本（GB18030）
  ├── novel.json         标准化原文，1200 个分析单元，后续一切的输入
  └── novel.index.json   轻量目录（不含正文），做规划时加载这个省内存

data/            机器产出的数据资产，删了能重跑（data/manual/ 例外）
  ├── chapters/          逐章分析，一章一个 json，共 1200 个（s2 / s2m / s5）
  ├── characters/        角色资产 997 个 + index.json（s4）
  ├── scenes/            场景资产 1364 个 + index.json（s4）
  ├── dossiers/          17 份角色原文卷宗的阅读稿（s8；机器读的 json 不入库，秒级可重跑）
  ├── plot/              剧情线、卷段、分集表、每集资产、按季清单（s0 / s4 / s9 ~ s13）
  └── manual/            ★ 人定的输入：seasons.json 六季定义、人工卷段、一次性分析产物

pipeline/        脚本管道，编号即执行顺序 —— 见 pipeline/README.md
  ├── s0 ~ s18           各步骤脚本，只有 s2 / s6 花 API 的钱
  ├── check_all.py       ★ 一键跑全部闸门（selftest / s3 / s14×47 / s15 / s17 / s18）
  ├── config.py          所有可调参数的唯一来源（读 .env）
  ├── common/            路径、命名、引用记法、质量闸门、逐字核验、LLM 客户端……
  ├── prompts/           四个提示词，纯文本，改了立即生效
  ├── schemas/           章节分析 JSON 的字段口径说明
  └── selftest.py        离线自测，196 条断言，不花额度

production/      人工产出的成片素材，与 data/ 的区别是「这些不可重新生成」
  ├── style_assets/      ★ 全片「怎么画」的唯一入口：统一风格 / 角色提示词 + 场景参考包
  ├── characters/        跨季人物视觉资产：CNN_ 具名卡、GNN_ 群像卡；_深度档案/ 14 份人写档案；_归档/
  ├── scenes/            跨季场景视觉资产：SNN_ 场景卡（母版 + 状态）
  ├── s01/               第一季：分集方案、总则、角色 / 场景取证、视觉基准补充、47 集剧本与幕文档、E01 出图台账
  └── style_test/        画风选型矩阵的配置与留痕（选型已结束）

doc/             文档 00~05、08~11、13~16；archive/ 放选型期的 06、07、12
CHANGELOG.md     变更与决策记录
.run/            运行期日志与体检报告，不入库
```

**`data/` 与 `production/` 的分界线**：`data/` 全部是脚本算出来的，删了能重跑；
`production/` 是人（或模型逐字读原文）写出来的，删了就没了。
所以改 `production/` 里的东西要当心，改 `data/` 里的东西一般应该去改产生它的脚本。
`data/manual/` 是唯一的例外——它装的是人定的输入，脚本只读不写。

---

## 快速上手

```bash
pip install -r requirements.txt
cp .env.example .env              # 填 DEEPSEEK_API_KEY / IMAGE_API_KEY，.env 不入库
python pipeline/check_all.py --quick   # selftest + s3 + s15 + s17，约一分钟，不花钱
python pipeline/check_all.py           # 再加 47 集的 s14 与出图台账 s18，约 70 秒
```

期望：selftest / s3 / s15 / s17 / s18 / s14 全绿。`s3` 单列的「人工放行 4 章」是人读过后接受的，不是问题。

密钥只从 `.env` 或环境变量读，**任何时候都不写进代码、不进版本库**。

---

## 已知欠账

按轻重排。全部记在 [`doc/03_路线图.md`](doc/03_路线图.md)，这里只列会让闸门变红或让人误判的：

1. **第 1115~1205 章之间 41 章正文被截**，影响第六季。见 [`doc/11`](doc/11_待补原文清单.md)。
2. **卷段是算法版**（90 段，只约六成边界站得住），只作分集的软约束；人工卷段只做了 seq201~400 一块。见 [`doc/08`](doc/08_全书剧情线.md) 4.5。
3. **第一季只有 E01 有分镜草图**，画面提示词层写到 E10。
4. **E01 的 20 张角色卡、23 张场景卡通过人工验收 0 张**，验收通过前不许开工出图（`doc/16` 第二节）。

---

## ⚠ 两件必须先知道的事

**一、全书不是 1232 章，是 1222 个章节标题、1200 个分析单元。**
作者自己有 14 处重号、5 处跳号；站点又把部分章节合并在一页。
所以全项目的主键是文件顺序 `seq`（1~1200），不是作者的章号。
见 [`doc/01_原文数据说明.md`](doc/01_原文数据说明.md) 第二节。

**二、第 1115~1205 章之间有 41 章的正文被截了，缺约 5.0 万字。**
影响第六季（大结局），第 1~1114 章与第 1206~1213 章完整，不影响前五季开工。
要补哪些章、补完怎么重跑，见 [`doc/11_待补原文清单.md`](doc/11_待补原文清单.md)。
